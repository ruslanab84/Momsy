...
struct AddFeedingSheet: View {
    ...
    var body: some View {
        ...
        .presentationBackground(Color.bbCoral)
    ...
    private var sheetHeader: some View {
        VStack(spacing: 18) {
            Capsule()
                .fill(FeedingPosterPalette.ink.opacity(0.16))
                .frame(width: 42, height: 5)
                .padding(.top, 10)

            HStack(spacing: 12) {
                headerButton(title: loc.strings.cancel, systemImage: "xmark", foreground: FeedingPosterPalette.inkSoft) {
                    dismiss()
                }
                ...
                headerButton(title: loc.strings.save, systemImage: "checkmark", foreground: isValid ? Color.bbCoralDeep : FeedingPosterPalette.inkMute) {
                    save()
                }
                .disabled(!isValid)
        ...
        .background {
            ZStack {
                ...
                FeedingOrganicBlob(variant: .topLeading)
                    .fill(Color.bbCoral.opacity(0.26))
    ...
    private var timeCard: some View {
        ...
        .padding(14)
        .background(FeedingPosterPalette.paper.opacity(0.96))
    ...
    private func sideButton(_ option: FeedingSide) -> some View {
        ...
        return Button {
            ...
            .frame(maxWidth: .infinity)
            .frame(height: 92)
            .background(isSelected ? tint.opacity(0.10) : FeedingPosterPalette.paperSoft.opacity(0.72))
            .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    .stroke(isSelected ? tint.opacity(0.28) : FeedingPosterPalette.ink.opacity(0.05), lineWidth: 1)
            )
        }
