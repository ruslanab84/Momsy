...
enum WeeklyInsightPrompt {
    ...
    static func system(for language: Language) -> String {
        ...
        - WHO recommends milk feeding on demand, day and night; it does NOT define one universal numeric
        ...
        - For babies younger than 6 months, never recommend complementary foods, solids, new foods,
          allergens, water, or "introduce one new food". Discuss breast milk or formula only.
    ...
    private static func feedingStageLine(for stats: WeeklyStats) -> String {
        if stats.ageMonths < 6 {
            return "FEEDING STAGE: under 6 months. WHO guidance is exclusive milk feeding for the first 6 months and feeding on demand, day and night; do not recommend solids or a fixed WHO feed count."
        }
    ...
    private static func feedingComparisonLine(for stats: WeeklyStats) -> String {
        if stats.ageMonths < 6 {
            ...
            return "MILK-FEED COMPARISON: \(decimal(actual))/day is at or above the app logging heuristic of about \(expectedMinimum)+ feeds/day (maximum interval about \(maxInterval) minutes). WHO still recommends feeding responsively on demand rather than targeting a fixed count."
        }
