...
final class GeminiWeeklyInsightService: WeeklyInsightService {
    ...
    /// Latest stable Flash model: better instruction following than Flash-Lite for a
    /// multi-section, safety-sensitive narrative while still supporting structured JSON.
    private let modelName = "gemini-3.6-flash"
    private let maxOutputTokens = 3_000
    ...
    /// one short sentence while keeping the persisted/UI model as joined strings.
    private static func responseSchema(for language: Language) -> Schema {
        ...
        return Schema.object(
            properties: [
                "sleepSummary": .array(
                    items: sentence,
                    description: "4 to 6 sentences covering logged sleep, the full WHO range, exact status/difference, night/day split, naps, trend, and a logging caveat.",
                    minItems: 4,
                    maxItems: 6
                ),
    ...
    /// Structured sentence arrays are the current response format. The legacy string
    /// format remains accepted for development fixtures and graceful SDK rollback.
